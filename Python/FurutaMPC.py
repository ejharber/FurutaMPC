...
def CostFunction(u):
    ...
    for ct in range(len(u)):
        ...
        for i in range(len(xk1)):
            J += (xk1[i]-xref[i])*Q[i]*(xk1[i]-xref[i])
...
cons = ({'type': 'ineq', 'fun': Contraints})
# contraint jacobian can also be added to possibly speed up 
tHistroy = []
t = time.time()

for ct in range(floor(Duration/Ts)):
    ...
    res = minimize(CostFunction, uopt, method='SLSQP', bounds=bnds, constraints=cons) 
    # jac=func_deriv arg that could possibly speed up time

    uopt = res.x
    x = IntegrationEstimation(x, uopt[0], Ts, 30)
    u0 = uopt[0]
    ...
    tHistroy.append(time.time() - t)

    # print(uopt) # for debugging purposes
    # print(x) 


# Displaying Graphs 
x = np.linspace(0, Duration, Duration/Ts+1)
x1 = np.linspace(1, floor(Duration/Ts), floor(Duration/Ts))
